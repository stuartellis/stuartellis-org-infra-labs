module "bastion_autoscaling_group" {
  ...
  iam_instance_profile_arn = module.iam_assumable_role_ec2_bastion.iam_instance_profile_arn

  security_groups = [
    aws_security_group.compute_bastion.id,
  ]

  service_linked_role_arn = time_sleep.bastion_iam_linked_role_creation.triggers["service_role"]

  vpc_zone_identifier = [var.ec2_network_config["subnet_id"]]
  image_id            = var.ec2_instance_config["image_id"]

  user_data = data.cloudinit_config.ec2_bastion_ubuntu.rendered
